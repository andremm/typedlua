...
-- typeArgList : (number, type?, boolean) -> (type)
function tlast.typeArgList (pos, t, strict)
  if not strict then
...
-- typeRetList : (number, type?, boolean) -> (type)
function tlast.typeRetList (pos, t, strict)
  if not strict then
...
function tlast.typeFunction (pos, t1, t2, is_method)
  if is_method then
    ...
    if t1.tag == "Tuple" then
      table.insert(t1, 1, s)
    else
      t1 = { tag = "Tuple", pos = pos, [1] = s }
    end
  end
