...
& | \; \{\overline{K:V}\}_{unique|open|fixed|closed} & \textit{table types}\\
...
\dfrac{\begin{array}{c}
	...
	\end{array}}
{\begin{array}{c}
	\senv \vdash \{\overline{K:V}\}_{fixed|closed} \subtype
	\{\overline{K^\prime:V^\prime}\}_{closed}
	\end{array}}
\end{array}\\\\
\begin{array}{c}
\mylabel{S-TableUtoC}\\
\dfrac{\begin{array}{c}
	...
	\end{array}}
{\senv \vdash \{\overline{K:V}\}_{unique} \subtype
	\{\overline{K^\prime:V^\prime}\}_{closed}}
\end{array}\\\\
\begin{array}{c}
\mylabel{S-TableOtoC}\\
\dfrac{\begin{array}{c}
	...
	\senv \vdash K_{i} \subtype K_{j}' \to \senv \vdash \Nil \subtype_{o} V_{j}'
	\end{array}}
{\senv \vdash \{\overline{K:V}\}_{open} \subtype
	\{\overline{K^\prime:V^\prime}\}_{closed}}
\end{array}\\\\
\begin{array}{c}
...
{\begin{array}{c}
	\senv \vdash \{\overline{K:V}\}_{unique} \subtype
	\{\overline{K^\prime:V^\prime}\}_{unique|open|fixed}
	\end{array}}
\end{array}\\\\
\begin{array}{c}
...
{\begin{array}{c}
	\senv \vdash \{\overline{K:V}\}_{open} \subtype
	\{\overline{K^\prime:V^\prime}\}_{open|fixed}
	\end{array}}
\end{array}\\\\
\begin{array}{c}
\mylabel{S-TableFtoF}\\
\dfrac{\begin{array}{c}
	...
	\senv \vdash V_{i} \subtype_{c} V_{j}'\\
	\end{array}}
{\senv \vdash \{\overline{K:V}\}_{fixed} \subtype
	\{\overline{K^\prime:V^\prime}\}_{fixed}}
\end{array}
\end{array}
\]
...
\dfrac{\begin{array}{c}
\overline{\env_1, \penv \vdash e_2: F, \env_v}
	\;\;\;
	\overline{\env_v, \penv \vdash e_1: K, \env_f}
	\\
	T = \{\overline{K{:}fix(F)}\}_{unique} \;\;\;
...
\dfrac{\begin{array}{c}
	\env_{1}(id) = \{ \overline{K:V} \}_{unique}\\
	\env_{1}, \penv \vdash k:K^{new}, \env_{2} \;\;\;
	\overline{K^{new} \not\lesssim K} \;\;\;
	V^{new} = fix(V)
	\end{array}}
{\begin{array}{l}
	\env_{1}, \penv \vdash id[k] {<}V{>}:V^{new},
	\env_{2}[id \mapsto \{ \overline{K{:}V}, K^{new}{:}V^{new}\}_{unique}]
	\end{array}}
\end{array}\\\\
\begin{array}{c}
\mylabel{T-IdRead}\\
...
\mylabel{T-IndexId}\\
\dfrac{\begin{array}{c}
	\env_{1}(id) = \{\overline{K:V}\} \;\;\;
	\env_{1}, \penv \vdash e_{2}:K^e, \env_{2} \;\;\;
	 K^e \lesssim K_{i}
	\end{array}}
...
\mylabel{T-IndexExp}\\
\dfrac{\begin{array}{c}
	\env_{1}, \penv \vdash e_{1}:\{\overline{K:V}\}, \env_{2}\\
	\env_{2}, \penv \vdash e_{2}:K^e, \env_{3} \;\;\;
	 K^e \lesssim K_{i}
	\end{array}}
...
\\\\
	...
	merget(T_1, T_2) & = & T_1 \;\;\; \mathrm{if\;} T_2 \subtype T_1\\
	merget(T_1, T_2) & = & T_2 \;\;\; \mathrm{if\;} T_1 \subtype T_2\\
	\multicolumn{3}{c}{\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\mathrm{the\;next\;case\;applies\;if}\;\forall i\;V^l_i\subtype_u V^r_i \,\vee\,V^r_i\subtype_u V^l_i \;\mathrm{and\;the\;right\; side\; is\;}wf}\\
	merget(\{\overline{K:V^l},\overline{K^{\prime}:V^\prime}\}_{unique},
	\{\overline{K:V^r},\overline{K^{\prime\prime}:V^{\prime\prime}}\}_{unique}) &=&
	\{\overline{K:sup_u(V^l,V^r)},\overline{K^\prime:V^\prime},\overline{K^{\prime\prime}:V^{\prime\prime}}\}_{unique}
	\\
	\multicolumn{3}{c}{\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\mathrm{the\;next\;case\;applies\;if}\;\forall i\;V^l_i\subtype_c V^r_i \,\vee\,V^r_i\subtype_c V^l_i
		\;\mathrm{and\;the\;right\; side\; is\;}wf}\\
	merget(\{\overline{K:V^l},\overline{K^{\prime}:V^{\prime}}\}_{unique|open},
	\{\overline{K:V^r},\overline{K^{\prime\prime}:V^{\prime\prime}}\}_{unique|open}) &=&
	\{\overline{K:sup_c(V^l,V^r)},\overline{K^\prime:V^\prime},\overline{K^{\prime\prime}:V^{\prime\prime}}\}_{open}
	\\
	merget(T_1, T_2) & = & \bot \;\;\; \mathrm{otherwise}\\\\
	join(\env_1,\env_2) & = & \{\overline{(id,joint(\env_1(id),\env_2(id))}\}\\
	joint(T_1, T_2) & = & T_1 \;\;\; \mathrm{if\;} T_2 \subtype T_1\\
	joint(T_1, T_2) & = & T_2 \;\;\; \mathrm{if\;} T_1 \subtype T_2\\
	\multicolumn{3}{c}{\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\mathrm{the\;next\;case\;applies\;if}\;\forall i\;V^l_i\subtype_u V^r_i \,\vee\,V^r_i\subtype_u V^l_i
		\;\mathrm{and\;the\;right\; side\; is\;}wf}\\
	joint(\{\overline{K:V^l},\overline{K^{\prime}:V^{\prime}}\}_{unique},
	\{\overline{K:V^r},\overline{K^{\prime\prime}:V^{\prime\prime}}\}_{unique}) &=&
	\{\overline{K:sup_u(V^l,V^r)},\overline{K^\prime:V^\prime\cup\Nil},\overline{K^{\prime\prime}:V^{\prime\prime}\cup\Nil}\}_{unique}\\
	joint(T_1, T_2) & = & \bot \;\;\; \mathrm{otherwise}\\\\
	sup_u(V_1, V_2) & = & V_2 \;\;\; \mathrm{if}\; V_1 \subtype_u V_2\\
	sup_u(V_1, V_2) & = & V_1 \;\;\; \mathrm{if}\; V_2 \subtype_u V_1\\\\
	reopen(\{\overline{K:V}\}_{closed}) & = & \{\overline{K:V}\}_{open} \\
	reopen(F) & = & F
	\end{array}
	\]}
...
necessary. This limitation of the type system is the reason the {\sc T-Constructor} rule uses the auxiliary function $fix$ to change any {\em open} and {\em unique} table types used in the field initializer to {\em fixed}. As an example, the table constructor $\{[``x"] = 1, [``y"] = \{[``z"] = 2\}\}$
...
in Figure~\ref{fig:tablerules} due to space. The only thing that
...
a table has been aliased it its identifier appears in r-value position, with a couple of exceptions which we will explain shortly.
...
& \multicolumn{2}{l}{$\mathbf{local} \; b:\{\}_{closed} = a \; \mathbf{in}$}\\
& & $\;a[``x"] \; {<}\String{>} = ``foo";$
\end{tabular}
\end{center}
...
alias is when indexing the table. Rule {\sc T-IndexId}
covers this case. For comparison, rule {\sc T-Index}
...
and $frv$ are straightforward, and were omitted due to lack of space.
