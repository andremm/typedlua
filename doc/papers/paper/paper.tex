...
the following example, where the {\tt newshape} function
is a constructor for objects with {\tt x} and {\tt y}
fields and a {\tt move} method:
\begin{verbatim}
  function new_shape(x, y)
    local shape = {}
    shape.x = x
    shape.y = y
...
functions with flexible arity, which use other variadic types,
and can use $\Value{*}$ if they want to just ignore extra
arguments.
