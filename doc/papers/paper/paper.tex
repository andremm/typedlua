...
Notice that in the above example the local variables $q$ and $r$ are declared without 
...
as rule {\sc T-ExpListProj} below shows:
\[
\begin{array}{c}
...
In the above example, the combination of these two rules will bind $x$ to
...
branches of the {\bf if} statement. The particular uses the rule {\sc T-IfFilterNilProj} below, which uses flow typing:
\[
%\begin{array}{c}
%\mylabel{T-IfFilterNilProj}\\
%\dfrac{\begin{array}{c}
...
Rule \textsc{T-IfFilterNilProj} uses the auxiliary functions \emph{fpt} and \emph{gpt}
