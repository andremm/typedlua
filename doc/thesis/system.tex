...
Multiple assignments also appear in function and method applications,
but our type system needs different typing rules for
expression lists that appear in function and method applications,
as we also want to catch arity mismatch.
For instance, assuming that $x$ and $f$ are in the environment with
types $\Integer \cup \Nil$ and
...
To do that, the rule \textsc{T-APPLY1} uses rules that type check
expression lists, but that do not include $\Nil{*}$ in the end of
the tuple type.
Instead, it uses the predicate $adjust$ to replace any missing
value with $\Nil$:
\[
\begin{array}{c}
\mylabel{T-APPLY1}\\
