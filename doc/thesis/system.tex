...
Our previous example type checks through these rules because
\[
...
produces an union of second-level types $S_{2}$ that sould be bound
...
This means that $q$ uses $\pi_{1}^{x}$ to project its type into the
union type $\Integer \cup \Nil$, while $r$ uses $\pi_{2}^{x}$ to
project its type into the union $\Integer \cup \String$.
