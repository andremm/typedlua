...
We use $\penv[\pi \mapsto r]$ for meaning that we extendend the environment
$\penv$ with the projection $\pi$ that maps to the union of tuple types $r$.
...
\begin{array}{c}
\mylabel{T-EXPLIST3}\\
\dfrac{\env \vdash me:t_{1} \times ... \times t_{n} \times \Void, \env_{1}}
...
x, y, z = f()_{m}
\]
assuming that $x$, $y$, $z$, and $f$ are in the environment with
types $\Integer$, $\String$, $\String \cup \Nil$, and
$\Void \rightarrow \Integer \times \String \times \Void$, respectively.
It type checks because
\[
...
Multiple assignments also appear in function applications, and the
rule \textsc{T-APPLY1} uses the same behavior from multiple assignments
to drop extra parameters and replace missing parameters:
\[
\begin{array}{c}
\mylabel{T-APPLY1}\\
\dfrac{\env_{1} \vdash e:r_{1} \rightarrow r_{2}, \env_{2} \;\;\;
       \env_{2} \vdash el:r_{3}, \env_{3} \;\;\;
       r_{3} \lesssim r_{1}}
      {\env_{1} \vdash e(el)_{m}:r_{2}, \env_{3}}
\end{array}
\]

Assuming that $x$ and $f$ are local variables that are in the environment
with types $\Integer \cup \Nil$ and
...
However, we also expect from a type system that it catches arity mismatch.
For this reason, our type system also has different typing rules for
expression lists that appear in function applications.
These rules do not include $\Nil{*}$ when the tuple type does not end
in a variadic type.
The rule \textsc{T-APPLY1-STRICT} use these rules to type check
expression lists, and it also uses the predicate $adjust$ to replace
any missing value with $\Nil$:
\[
\begin{array}{c}
\mylabel{T-APPLY1}\\
\dfrac{\env_{1} \vdash e:r_{1} \rightarrow r_{2}, \env_{2} \;\;\;
       \env_{2} \vdash el:r_{3}, \env_{3} \;\;\;
       r_{4} = adjust(r_{3}, r_{1}) \;\;\;
       r_{4} \lesssim r_{1}}
      {\env_{1} \vdash e(el)_{m}:r_{2}, \env_{3}}
\end{array}
\]
...
\emph{adjust} as follows:
\begin{align*}
adjust(\Void, t{*}) & = \Nil \times \Void\\
adjust(\Void, t \times s) & = \Nil \times adjust(\Void,s)\\
adjust(t_{1} \times s_{1}, t_{2} \times s_{2}) & = t_{1} \times adjust(s1,s2)\\
adjust(s_{1}, s_{2}) & = s_{1}
\end{align*}

Now, assuming that $f$ has type
$\String \times \Integer \cup \Nil \times \Integer \cup \Nil \times \Void \rightarrow \Integer{*}$,
the program
\[
x = f(``foo")_{m}
\]
type checks through the rule \textsc{T-APPLY1-STRICT} because
\[
``foo" \times \Nil \times \Nil \times \Void \lesssim \String \times \Integer \cup \Nil \times \Integer \cup \Nil \times \Void
\]
but the program
\[
x = f(``foo",1,2,3)_{m}
\]
does not type check through the rule \textsc{T-APPLY1-STRICT} because
\[
``foo" \times 1 \times 2 \times 3 \times \Void \not\lesssim \String \times \Integer \cup \Nil \times \Integer \cup \Nil \times \Void
\]

The same happens with the rules that type check the \textbf{return}
statement.
The rule \textsc{T-RETURN} uses $\Nil{*}$ to replace eventual missing
values, while the rule \textsc{T-RETURN-STRICT} uses the predicate
\emph{adjust} to replace values that are certainly missing:
\[
\begin{array}{c}
\begin{array}{c}
\mylabel{T-RETURN}\\
...
\\ \\
\begin{array}{c}
\mylabel{T-RETURN-STRICT}\\
\dfrac{\env_{1} \vdash el:r_{1}, \env_{2} \;\;\;
       \env_{2}(\ret) = r_{2} \;\;\;
       r_{3} = adjust(r_{1}, r_{2}) \;\;\;
       r_{3} \lesssim r_{2}}
      {\env_{1} \vdash \mathbf{return} \; el:\env_{2}}
\end{array}
\end{array}
...
to the type checking rule of the first form, and just controls the type of
the omitted indices.
...
if our table types allowed any first-level type in the keys.
...
\dfrac{\env_{1} \vdash e_{1}:t_{1}, \env_{2} \;\;\;
       \env_{2} \vdash e_{2}:t_{2}, \env_{3}}
      {\env_{1} \vdash [e_{1}] = e_{2}: \Value{:}close(t_{2}), \env_{3}}
\end{array}
\]

The rule \textsc{T-CONSTRUCTOR2} along with the rules for table fields
assign the type $\{``x":1, ``y":\{``z":2\}_{closed}\}_{unique}$ to
the table constructor $\{[``x"] = 1, [``y"] = \{[``z"] = 2\}\}$.
...
the rule \textsc{T-CONSTRUCTOR3} gives the type $\{\Integer:\String \cup \Nil\}_{unique}$
to the table constructor $\{{...}_{m}\}$.
...
If we assume that the $...$ is in the environment and has type $\String$,
the rule \textsc{T-CONSTRUCTOR4} gives the type
$\{``x":``foo", ``y":\mathbf{true}, \Integer:\String \cup \Nil\}_{unique}$
to the table constructor $\{[``x"] = ``foo", [``y"] = \mathbf{true}, {...}_{m}\}$.
As another example, the rule \textsc{T-CONSTRUCTOR4} fails to type check
...
\dfrac{\begin{array}{c}
       ...
       \env_{1} \vdash c:k_{n+1}, \env_{2} \;\;\;
       \not \exists i \in 1..n \; t_{1} \lesssim k_{i} \;\;\;
       v_{n+1} = close(t)
       \end{array}}
      {\env_{1} \vdash n[c] {<}t{>}:t, \env_{2}[n \mapsto \{ k_{1}{:}v_{1}, ..., k_{n}{:}v_{n}, k_{n+1}{:}v_{n+1}\}_{open|unique}]}
\end{array}
\]
...
\end{tabular}
\end{center}
does not type check because aliasing $a$ produces the type
$\{``foo":\Integer\}_{closed}$ that is not subtype of
$\{``foo":\Integer \cup \Nil\}_{closed}$, the type of $b$.
The rules \textsc{T-IDREAD1} and \textsc{T-IDREAD2} define this behavior:
\[
\begin{array}{c}
\begin{array}{c}
\mylabel{T-IDREAD1}\\
...
\begin{array}{c}
\mylabel{T-IDREAD2}\\
\dfrac{\env_{1}(n) = \{k_{1}{:}v_{1}, ..., k_{n}{:}v_{n}\}_{open}}
      {\env_{1} \vdash n_{e}:\{k_{1}{:}v_{1}, ..., k_{n}{:}v_{n}\}_{closed}, \env_{1}}
\end{array}
\end{array}
\]

Aliasing \emph{unique} and \emph{open} table types can be unsafe.
For this reason, the rules \textsc{T-IDREAD1} and \textsc{T-IDREAD2}
promote the type of an alias expression from \emph{unique} or \emph{open}
to \emph{closed}.
The rule \textsc{T-IDREAD1} also promotes the aliased variable from
\emph{unique} to \emph{open}, as \emph{unique} table types only
denote the type of table types that have no alias.
...
We also have different rules for type checking table indexing that
appear as expressions to avoid promoting an \emph{unique} table to
\emph{open} in these operations:
\[
\begin{array}{c}
\begin{array}{c}
\mylabel{T-INDEXREAD1}\\
\dfrac{\env_{1}(n) = \{k_{1}{:}v_{1}, ..., k_{n}{:}v_{n}\} \;\;\;
       \env_{1} \vdash e_{2}:t, \env_{2} \;\;\;
       \exists i \in 1{..}n \; t \lesssim k_{i}}
      {\env_{1} \vdash n_{e}[e_{2}]:v_{i}, \env_{2}}
\end{array}
\\ \\
\begin{array}{c}
\mylabel{T-INDEXREAD1}\\
...
\dfrac{\env_{1}(n) \subtype t}
      {\env_{1} \vdash {<}t{>} \; n:close(t), \env_{1}[n \mapsto t]}
\end{array}
\]
...
\begin{array}{c}
\mylabel{T-EXPLIST6}\\
\dfrac{\env \vdash me:t_{1} \times ... \times t_{n} \times \Void \sqcup t_{1}' \times ... \times t_{n}' \times \Void, \env_{1}}
      {\env \vdash me:\pi_{1} \times ... \times \pi_{n} \times \Nil{*}, \env_{1}}
\end{array}
\]
