...
to use the consistent-subtyping relation to check multiple assignments:
\[
\begin{array}{c}
...
Its inference rule is starightforward:
\[
\begin{array}{c}
...
multiple expressions that can return a tuple type that does not end
...
check if the type of the key is a $\Boolean$, a $\Number$,
...
and thus do not allow chaning the value that is stored in the field $``x"$
of local $a$.
