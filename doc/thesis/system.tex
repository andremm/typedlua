...
\textit{/* \textbf{begin} TODO list */}

\begin{itemize}
\item Refinement with \_ENV
\item Assignment + left-hand side
\item Locals + inference rules + unique and closed table types
\end{itemize}

\textit{/* \textbf{end} TODO list */}
...
\textit{/* Should we split into two rules instead of using $open|unique$? */}
...
We use the refinement of table types to handle the declaration of global variables.
In Lua, the assignment \texttt{v = 5} is syntactic sugar to \texttt{\string_ENV["v"] = 5}
when \texttt{v} is not a local variable, where \texttt{\string_ENV} is a table
...
We are assuming that $\string_ENV$ is in the environment and has type $\{\}_{open}$.
...
in $\string_ENV$.
We could allow the refinement of existing fields, but this could
lead to fields that have a too general type, which could decrease
the ammout of type checking.
