...
we could have ommited the reflexive rules \textsc{S-LITERAL} and
...
Table types have only one syntactical form, but they also have one of
these three tags: \emph{closed}, \emph{open}, and \emph{unique}.
The tag \emph{closed} is the tag of the table types that the type
annotations describe.
The tag \emph{open} is the tag of the table types that we can
add new fields through field assignment.
The tag \emph{unique} is the tag of the table types that represent
the type of the table constructors.
The syntactical form of table types is $\{ v_{i}, ..., v_{n} \}$,
where $i$ varies from 1 to $n$ when $n > 0$,
but $i$ can also be 0 when $n = 0$.
