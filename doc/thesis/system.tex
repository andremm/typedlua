...
The simples expression involving tables in the empty table constructor.
...
\mylabel{T-IDREAD}\\
\dfrac{\env(n) = t}
      {\env, \penv \vdash n_{e}:close(t), \env[n \mapsto open(t)]}
\end{array}
\]
...
\mylabel{T-IDWRITE}\\
\dfrac{\env(n) = t}
      {\env, \penv \vdash n_{l}:close(t), \env[n \mapsto close(t)]}
\end{array}
\]
...
changing table types in these operations:
\[
\begin{array}{c}
\begin{array}{c}
\mylabel{T-INDEX1}\\
...
table type to initialize a \emph{regular} table type, as \emph{unique}
table types allow us to initialize optional fields.
To do that, Typed Lua includes a coercion expression that allows us
to use an \emph{unique} table type before it becomes \emph{open}.
The rule \textsc{T-COERCION} describes this behavior:
\[
\begin{array}{c}
\mylabel{T-COERCION}\\
...
The rule \textsc{T-COERCION} allows our type system to type check the following example:
\begin{center}
\begin{tabular}{lll}
\multicolumn{3}{l}{$\mathbf{local} \; a:\{\}_{unique} = \{ \} \; \mathbf{in}$}\\
...
We can use $a$ to initialize $b$ because the coercion expression converts
...
The function \emph{closeall} closes all \emph{open} and \emph{unique}
...
\dfrac{\begin{array}{c}
       ...
       closeall(\env_{1}[self \mapsto \Self, \vec{n} \mapsto \vec{t}]), \penv[\ret \mapsto r] \vdash s, \env_{2}\\
...
\begin{center}
\begin{tabular}{llll}
\multicolumn{4}{l}{$\mathbf{local} \; s:\{``x":\Number, ``y":\Number\}_{unique} = \{ [``x"] = 0.0, [``y"] = 0.0 \}$}\\
...
Then, it uses two method definitions to include $new$ and $move$ into our class.
At the end of the example, $s$ has type
\begin{align*}
\{ & ``x":\Number, ``y":\Number,\\
...
to initialize the $s$ with type $\Self$.
