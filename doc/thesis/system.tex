...
\begin{array}{c}
\mylabel{T-IDREAD1}\\
\dfrac{\env(id) = T_{1} \;\;\; T_{2} = read(\penv, T_{1})}
      {\env, \penv \vdash id:close(T_{2}), \env[id \mapsto open(T_{1})]}
\end{array}
\;
\begin{array}{c}
\mylabel{T-IDREAD2}\\
\dfrac{\env(id) = T_{1} \;\;\; T_{2} = read(\penv, T_{1})}
      {\env, \penv \vdash id:fix(T_{2}), \env[id \mapsto fix(T_{1})]}
\end{array}
\end{array}
\]
...
\begin{array}{c}
\mylabel{T-IDWRITE}\\
\dfrac{\env(id) = T_{1} \;\;\; T_{2} = write(T_{1})}
      {\env, \penv \vdash id_{l}:close(T_{2}), \env[id \mapsto close(T_{2})]}
\end{array}
\]
