...
Our example uses the rule \textsc{T-EXPLIST1} which is the rule
that type checks an expression list where all expressions can
...
Then, our type system type checks the list of left-hand values.
...
expected in the left-hand side.
Our example uses the rule \textsc{T-LHSLIST} which is the rule that type
...
\dfrac{\begin{array}{c}
       \env_{i}, \penv \vdash ([e_{1}] = e_{2})_{i}:(K_{i},V_{i}), \env_{i+1} \\
       T = \{K_{1}{:}V_{1}, ..., K_{n}{:}V_{n}\}_{unique} \;\;\;
       wf(T) \;\;\;
       n = |\;\overline{[e_{1}] = e_{2}}\;|
       \end{array}}
      {\env_{1}, \penv \vdash \{\;\overline{[e_{1}] = e_{2}}\;\}:T, \env_{n+1}}
\end{array}
\]
...
In case of failure, it returns $\Nil$ plus an error message that indicates
the error.
...
Rule \textsc{T-LOCAL2} captures this intuition:
\[
\begin{array}{c}
\mylabel{T-LOCAL2}\\
\dfrac{\begin{array}{c}
...
However, we want to discriminate these variables to check whether
