...
of annoatated variables, we use the latter to show how our type system
...
even if they return mutiple values.
...
\dfrac{\env, \penv \vdash e_{i}:T_{i}, \env_{i} \;\;\;
       ...
       n = |\;\overline{e}\;|}
      {\env, \penv \vdash \overline{e}:T_{1} \times ... \times T_{n} \times \Nil{*}, \env_{m}}
\end{array}
\\ \\
\begin{array}{c}
\mylabel{T-LHSLIST}\\
\dfrac{\env, \penv \vdash l_{i}:T_{i}, \env_{i} \;\;\;
       \env_{m} = merge(\env_{1}, ..., \env_{n}) \;\;\;
       n = |\;\overline{l}\;|}
      {\env, \penv \vdash \overline{l}:T_{1} \times ... \times T_{n} \times \Value{*}, \env_{m}}
\end{array}
\end{array}
\]
...
The previous example type checks through these rules because
\[
...
types to appear in the type of the value would allow the
...
environment and has type $\{\}_{open}$. For instance,
\[
...
Rules \textsc{T-IDREAD1} and \textsc{T-IDREAD2} define this behavior:
\[
\begin{array}{c}
\begin{array}{c}
\mylabel{T-IDREAD1}\\
...
\mylabel{T-IDREAD2}\\
\dfrac{\env(n) = T_{1} \;\;\; T_{2} = read(\penv, T_{1})}
      {\env, \penv \vdash n:fix(T_{2}), \env[n \mapsto fix(T_{1})]}
\end{array}
\end{array}
\]
...
of unions of first-level and second-level types.
...
\begin{array}{c}
\mylabel{T-FUNCTION3}\\
\dfrac{crall(\env_{1}[\vec{n} \mapsto \vec{T}]), \penv[\ret \mapsto S] \vdash s, \env_{2}}
      {\begin{array}{c}
       \env_{1}, \penv \vdash \mathbf{fun} \; (\vec{n{:}T}){:}S \; s:\vec{T} \times \Void \rightarrow S,\\
       openset(crset(\env_{1}, fav(\mathbf{fun} \; (\vec{n{:}T}){:}S \; s)),rv(\mathbf{fun} \; (\vec{n{:}T}){:}S \; s))
       \end{array}}
\end{array}
\]
...
table types; it also restores filter types to their original type.
...
while the function \emph{print} thas type
...
produces a union of second-level types $S_{2}$ that sould be bound
to a variable $x$ in the projection environment,
...
$(x,\Integer \times \Integer \times \Void \sqcup \Nil \times \String \times \Void)$. 

Our type system uses the pair $(x,S_{2})$ to allow rule
\textsc{T-LOCAL} to bound a union of second-level types $S_{2}$
to a variable $x$ in $\penv$:
\[
\begin{array}{c}
\mylabel{T-LOCAL}\\
\dfrac{\begin{array}{c}
...
In our example, $q$ gets type $\pi_{1}^{x}$ and $r$ gets type
$\pi_{2}^{x}$.
