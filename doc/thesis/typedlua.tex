...
type of the record we showed in the last section:
\begin{verbatim}
...
However, we limitim aliases to union types, function types, table types,
and recursive types.
...
alias to the type of the abstract syntax tree for a language of simple
...
deconstruct unions of table types, and it resembles
...
if the type of the variable \texttt{n} is a union of table types
and if \texttt{s1} and \texttt{s2} are strings.
If that is the case, it constrains the type of \texttt{n} inside
the \texttt{if} to the table type that has the pair \texttt{s1:s2}.
For instance, in the case of \texttt{e.tag == "Add"}, Typed Lua
constrains the type of \texttt{e} to \texttt{\{"tag":"Add", 1:Exp, 2:Exp\}}.
...
For this reason worth keeping two different constructs.
One for records specifically and another to more general types.
...
A type alias declaration assigns a name to a first-level type.
