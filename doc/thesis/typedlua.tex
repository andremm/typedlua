...
\textit{tabletypebody} ::= & \;\; \textit{hashtype} \; | \;
  \textit{recordtype}\\
\textit{hashtype} ::= & \;\; [\textit{keytype} \; \texttt{`:'}] \; \textit{type}\\
...
The refinement only affects the type of \texttt{x} until the
next assignment to \texttt{x}.
...
constrains the type of \texttt{x} from \texttt{string|t} to
