...
to initialize \texttt{Circle}, openning the type of \texttt{Circle}
to allow the refinement of table types to build the type of
the class \texttt{Circle}.

In a \texttt{setmetatable} expression
\texttt{setmetatable(e:(el),\{\string_\string_index = id\})},
if the expression \texttt{e:(el)} has a \emph{closed} table type \texttt{t},
