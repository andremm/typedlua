...
%       closeall(\env_{1})[self \mapsto \Self, \overline{id \mapsto F}, \self \mapsto F_{s}],
%       \penv[\ret \mapsto S] \vdash s, \env_{3}\\
%       V_{i} = \Const \; \Self \times F_{1} \times ... \times F_{n} \times \Nil{*} \rightarrow S\\
%       \env_{4} = openset(crset(\env_{1}[id_{1} \mapsto F_{o}], fav(\mathbf{fun} \; (\overline{id{:}T}){:}S \; s)),rv(\mathbf{fun} \; (\overline{id{:}T}){:}S \; s))
%       \end{array}}
%      {\begin{array}{c}
