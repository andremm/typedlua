...
some Lua features and idioms that helped us identify how should
we combine static and dynamic typing in Lua.
...
each language ends up implementing its optional type system on
its own way.
...
The rule \textsc{C-FUNC} resembles the subtyping between function types,
...
we did not combine these rules in a single one to make explicit the
...
The consistent-subtyping relation is not transitive, and that is the
reason why the dynamic type $?$ is not neutral to this relation.
...
to handle this issue, and \citet{ahmed2011bfa} extended the
...
There are some programming languages that either incorporated
gradual typing or have research on this subject.
...
Typed Lua should also include some way for defining the types of
userdata.
This feature should also allow programmers to define an userdata
