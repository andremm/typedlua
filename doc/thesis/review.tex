...
\begin{figure}[!ht]
\textbf{Syntax of the gradually-typed lambda calculus}\\
\dstart
$$
\begin{array}{rlr}
T ::= & & \emph{types}\\
...
$$
\begin{matrix}
\Number \lesssim \Number \mylabel{C-NUM} \;\;\; \String \lesssim \String \mylabel{C-STR}
\\ \\
T \lesssim \;? \mylabel{C-ANY1} \;\;\; ? \lesssim T \mylabel{C-ANY2}
\\ \\
\dfrac{T_{3} \lesssim T_{1} \;\;\; T_{2} \lesssim T_{4}}
      {T_{1} \rightarrow T_{2} \sim T_{3} \rightarrow T_{4}} \mylabel{C-FUN}
\;\;\;
\dfrac{\forall i \in 1..n \;\;\; T_{i} \lesssim T_{i}'}
      {[l_{i}:T_{i}^{\;i\in1..n+m}] \lesssim [l_{i}:T_{i}'^{\;i\in1..n}]} \mylabel{C-OBJ}
\end{matrix}
$$
\dend
