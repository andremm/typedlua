...
\frac{\env \vdash e:T_{1} \;\;\; T_{1} \subtype T_{2}}
     {\env \vdash e:T_{2}}
\]
\item However, we also aim to have gradual typing, so we also
...
\begin{frame}
\frametitle{Typed Lua Type System}
First, we do not use the subsumption rule, but we use subtyping
instead of equality:
\[
\frac{\env \vdash e:S_{1} \rightarrow S_{2} \;\;\;
     ...
     {\env \vdash e(el):S_{2}}
\]
Second, we also use the consistency rule:
\[
\frac{\env \vdash e:S_{1} \rightarrow S_{2} \;\;\;
      \env \vdash el:S_{3} \;\;\;
      S_{3} \subtype S_{1}' \;\;\;
     ...
     {\env \vdash e(el):S_{2}}
\]
Finally, we can use one relation that composes the two:
\[
\frac{\env \vdash e:S_{1} \rightarrow S_{2} \;\;\;
      \env \vdash el:S_{3} \;\;\;
      S_{3} \lesssim S_{1}}
     {\env \vdash e(el):S_{2}}
\]
which we can call consistent-subtyping.
\end{frame}
